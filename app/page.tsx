...
export default function Home(){
 ...
 const markSynced=(remoteCount:number,currentCount:number)=>{setSyncStatus(`GitHub ${remoteCount}건 · 화면 ${currentCount}건`);setSyncAt(new Intl.DateTimeFormat('ko-KR',{hour:'2-digit',minute:'2-digit'}).format(new Date()))};
 const syncNow=async()=>{setSyncStatus('GitHub 최신 데이터 확인 중…');try{const remote=await fetchRemote();const merged=mergeRecords(acts.slice(),remote);setActs(merged);markSynced(remote.length,merged.length)}catch(e){setSyncStatus(`동기화 실패: ${e instanceof Error?e.message:String(e)}`)}};
 ...
 useEffect(()=>{let cancelled=false;syncOnMount();async function syncOnMount(){try{const remote=await fetchRemote();if(cancelled)return;const raw=localStorage.getItem(STORAGE);let local:Activity[]=[];if(raw){try{const p=JSON.parse(raw);if(Array.isArray(p))local=p.map(normalizeRecord)}catch{}}const merged=mergeRecords(local,remote);if(!cancelled){setActs(merged);markSynced(remote.length,merged.length)}}catch{if(!cancelled)setSyncStatus('자동 동기화 실패 — 로컬 기록 유지')}}return()=>{cancelled=true}},[]);
