...
export default function Home(){
 ...
 return <main className={dark?'app dark':'app'}>
  ...
  {selected&&<div className="modal-backdrop" onClick={()=>setSelected(null)}><div className="modal" onClick={e=>e.stopPropagation()}><div className="modal-head"><div><div className="eyebrow">ACTIVITY DETAIL</div><h2>{selected.type} · {selected.distanceKm.toFixed(2)} km</h2><p>{fmtFullDate(selected.date)} · {fmtDur(selected.durationSec)}</p></div><button className="close" onClick={()=>setSelected(null)}>×</button></div><div className="metric-grid"><Metric label="평균 페이스" value={fmtPace(selected.avgPaceSecPerKm)}/><Metric label="최고 페이스" value={fmtPace(selected.bestPaceSecPerKm)}/><Metric label="평균 속도" value={selected.avgSpeedKmh?selected.avgSpeedKmh.toFixed(1):'—'} unit={selected.avgSpeedKmh?'km/h':undefined}/><Metric label="최고 속도" value={selected.bestSpeedKmh?selected.bestSpeedKmh.toFixed(1):'—'} unit={selected.bestSpeedKmh?'km/h':undefined}/><Metric label="평균 심박" value={selected.avgHeartRate?Math.round(selected.avgHeartRate):'—'} unit={selected.avgHeartRate?'bpm':undefined}/><Metric label="최대 심박" value={selected.maxHeartRate?Math.round(selected.maxHeartRate):'—'} unit={selected.maxHeartRate?'bpm':undefined}/><Metric label="케이던스" value={selected.avgCadence?Math.round(selected.avgCadence):'—'} unit={selected.avgCadence?'spm':undefined}/><Metric label="최대 케이던스" value={selected.maxCadence?Math.round(selected.maxCadence):'—'} unit={selected.maxCadence?'spm':undefined}/><Metric label="칼로리" value={selected.calories?Math.round(selected.calories):'—'} unit={selected.calories?'kcal':undefined}/><Metric label="걸음수" value={selected.steps?Math.round(selected.steps).toLocaleString():'—'}/><Metric label="VO₂ Max" value={selected.vo2max??'—'}/><Metric label="고도 상승" value={selected.elevationGainM?selected.elevationGainM.toFixed(1):'—'} unit={selected.elevationGainM?'m':undefined}/><Metric label="고도 하강" value={selected.elevationLossM?selected.elevationLossM.toFixed(1):'—'} unit={selected.elevationLossM?'m':undefined}/>
<Metric label="최저 고도" value={selected.minAltitudeM??'—'} unit={selected.minAltitudeM!=null?'m':undefined}/><Metric label="최고 고도" value={selected.maxAltitudeM??'—'} unit={selected.maxAltitudeM!=null?'m':undefined}/><Metric label="오르막 거리" value={selected.uphillDistanceKm!=null?selected.uphillDistanceKm.toFixed(2):'—'} unit={selected.uphillDistanceKm!=null?'km':undefined}/><Metric label="내리막 거리" value={selected.downhillDistanceKm!=null?selected.downhillDistanceKm.toFixed(2):'—'} unit={selected.downhillDistanceKm!=null?'km':undefined}/></div>{selected.note&&<div className="detail-note">메모: {selected.note}</div>}<div className="modal-actions"><button className="danger" onClick={()=>setConfirm({kind:'trash',id:selected.id})}>휴지통으로 이동</button><button className="primary" onClick={()=>setSelected(null)}>닫기</button></div></div></div>}
