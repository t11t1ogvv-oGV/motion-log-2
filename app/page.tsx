   ...
   {tab==='home'&&<>
    <section className="dashboard-calendar panel">
      ...
      <div className="section-head" style={{marginTop:0}}>
        <div>
          <h3>{currentMonth.year}년 {currentMonth.month+1}월</h3>
          <span className="calendar-summary">{currentMonthActivities.length}회 운동 · {currentMonthDist.toFixed(1)} km</span>
        </div>
        <div style={{display:'flex',gap:5}}>
          <button className="secondary" onClick={()=>setMonthOffset(v=>v-1)}>←</button>
          <button className="secondary" onClick={()=>setMonthOffset(0)}>오늘</button>
          <button className="secondary" onClick={()=>setMonthOffset(v=>v+1)}>→</button>
        </div>
      </div>
    ...
    <section className="home-week-summary panel">
      <div className="section-head" style={{marginTop:0}}>
        <div><div className="eyebrow">THIS WEEK</div><h3>이번 주</h3></div>
      </div>
      <div className="home-week-grid">
        <Metric label="운동" value={week.length} unit="회"/>
        <Metric label="거리" value={weekDist.toFixed(1)} unit="km"/>
        <Metric label="평균 페이스" value={weekAvgPace?fmtPace(weekAvgPace):'—'}/>
      </div>
    </section>

    <div className="section-head home-recent-head">
      <div><div className="eyebrow">RECENT</div><h3>최근 운동</h3></div>
      <button className="text-btn" onClick={()=>setTab('records')}>전체 보기 →</button>
    </div>
    <div className="records-list home-recent-list">
      {active.slice(0,5).map(a=><article className="home-recent-card" key={a.id} onClick={()=>setSelected(a)}>
        <div className="home-recent-main">
          <div className="record-date"><b>{fmtDate(a.date)}</b><span>{a.type}</span></div>
          <div className="record-main"><b>{a.distanceKm.toFixed(2)}<small> km</small></b><span>{fmtDur(a.durationSec)} · {fmtPace(a.avgPaceSecPerKm)}</span></div>
        </div>
        <div className="record-tags">
          {a.avgHeartRate&&<span>심박 {Math.round(a.avgHeartRate)}</span>}
          {a.avgCadence&&<span>케이던스 {Math.round(a.avgCadence)}</span>}
